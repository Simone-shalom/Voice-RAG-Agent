...
**Decyzja:** jeden workflow (`ci.yml`) z dwoma rownoleglymi jobami (`backend`, `frontend`), obydwa wymagane do merge na `main`. Bez sekretow API czy bazy danych — cala suita testów jest mockowana (300 testów backend, 48 testów frontend), nie wymaga real DB/API keys. Status badge w README.md linkuje do tego samego workflow file (`ci.yml`), zeby pozostal funkcjonalny niezaleznie od przyszlych rozszerzen.

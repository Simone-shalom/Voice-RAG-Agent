...
## [Etap 17] Jeden workflow GitHub Actions (`ci.yml`) z dwoma parallelymi jobami, bez sekretow (2026-09-12)

**Decyzja:** jeden workflow (`ci.yml`) z dwoma rownoleglymi jobami (`backend`, `frontend`), obydwa wymagane do merge na `main`. Bez sekretow API czy bazy danych — cala suita testów jest mockowana (244 testy backend, 48 testów frontend), nie wymaga real DB/API keys. Status badge w README.md linkuje do tego samego workflow file (`ci.yml`), zeby pozostal funkcjonalny niezaleznie od przyszlych rozszerzen.
...
**Uzasadnienie:** projekt osiagnal 348 testow (244 backend unit + 48 frontend component + 96 w eval) i zero automatyzacji ich uruchamiania — realna luka dla portfolio wozka do rekrutera. Kazdego pusha na `main` czy PR do `main` powinno teraz automatycznie sprawdzic, czy suita przechodzi (status badge na README jest wizualnym dowodem). Pierwszy przebieg workflow-u (commit 425c4ef) przeszedl bez zadnych poprawek — oba joba (`backend`, `frontend`) skonczyly sie z conclusion `success` w ciagu ~2 minut (backend ~60s, frontend ~90s, bez overlapa dzalko z `runs-on: ubuntu-latest` na dwoch osobnych VM w paraleli). Brak surprises w zaleznosciach czy dlugosci kompilacji — pytest i npm ci/test "just work" na ubutu juz za pierwszym razem.
